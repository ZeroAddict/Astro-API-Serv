...
exports.create = (req, res) => {
  ...
  createdForm.parse(req, (error, fields, files) => {
    ...
    const { name, type, desc, price, category } = fields;
    ...
    if (files.pic) {
      ...
      if (files.pic.size > 4096000) {
        return res.status(400).json({
          error: 'Photo size should be smaller than 4MB',
        });
      }
...
exports.list = (req, res) => {
  ...
  let api = findSearch.api ? findSearch.api : 'asc';
